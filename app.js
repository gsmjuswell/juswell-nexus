...
window.addEventListener("load", () => {
  // Masquer le splash à 3s quoi qu'il arrive
  setTimeout(() => {
    ...
    // Garantir qu'un écran est visible
...
$("btn-login").addEventListener("click", async () => {
  ...
  if (id === "DG" && pwd === "Juswell123") {
    currentUser = {
      uid:    "qwPKdQBO48fVgFqYpScSeN8ZSnm2",
      userId: "DG",
  ...
  try {
    ...
    if (snap.empty) {
      await addDoc(collection(db, "groups"), {
        ...
        ref: "",
        type: "permanent",
        members: ["qwPKdQBO48fVgFqYpScSeN8ZSnm2"],
        createdBy: "qwPKdQBO48fVgFqYpScSeN8ZSnm2",
        createdAt: serverTimestamp()
      });
